#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
#  🏎️ DORIKIN HELM TEST TRACK - Helm Chart Drift Testing Environment
# ═══════════════════════════════════════════════════════════════════════════════
#
#  A test harness for dorikin's Helm integration that:
#  - Uses the existing Colima k3s cluster (from test-track.sh setup)
#  - Deploys a test Helm chart to dorikin-pitlane namespace
#  - Creates intentional DRIFT scenarios to test Helm mode
#  - Provides interactive controls for testing
#
#  Requirements: Colima running (via test-track.sh setup), helm, kubectl
#
#  Usage:
#    ./scripts/test-track-helm.sh [command]
#
#  Commands:
...
#    scan       - Run dorikin scan with --helm flag
#    tui        - Launch dorikin TUI with --helm flag
...
# Helm test namespace (pitlane = pit crew / preparation)
TEST_NAMESPACE="dorikin-pitlane"

# Helm chart directory
HELM_CHART_DIR="${PROJECT_ROOT}/testdata/track-helm/dorikin-test"
...
EOF
    echo -e "  ${WHITE}🏎️  HELM TEST TRACK${NC} ${GRAY}// Pitlane Drift Testing${NC}"
    echo -e "${NC}"
}
...
preflight() {
    ...
    if command_exists helm; then
        substep "Helm: ${JADE}$(helm version --short 2>/dev/null)${NC}"
    else
        missing+=("helm")
        substep "Helm: ${RED}✗${NC}"
    fi
...
deploy() {
    banner
    section "Deploying Helm Chart to Pitlane"
    ...
    helm upgrade --install dorikin-test "${HELM_CHART_DIR}" \
        --namespace "${TEST_NAMESPACE}" \
        --kube-context "$(get_context)" \
    ...
    echo -e "${JADE}The Helm test track is ready!${NC}"
    echo ""
    echo -e "  Context:   ${CYAN}$(get_context)${NC}"
    echo -e "  Namespace: ${CYAN}${TEST_NAMESPACE}${NC}"
    echo -e "  Chart:     ${CYAN}${HELM_CHART_DIR}${NC}"
    echo ""
...
drift_extra() {
    info "Drift: Adding rogue deployment (not in chart)"
...
drift_all() {
    ...
    echo -e "${BG_YELLOW}${BLACK}  ⚡⚡⚡ PITLANE CHAOS MODE ⚡⚡⚡  ${NC}"
    echo ""
...
drift_menu() {
    ...
    echo -e "  ${JADE}7${NC}) Extra        ${GRAY}Add deployment not in chart${NC}"
...
reset() {
    banner
    section "Resetting Pitlane to Baseline"
    ...
    step "Redeploying chart..."
    kc create namespace "${TEST_NAMESPACE}"
    helm upgrade --install dorikin-test "${HELM_CHART_DIR}" \
        --namespace "${TEST_NAMESPACE}" \
        --kube-context "$(get_context)" &>/dev/null

    wait_ready deployment nginx "${TEST_NAMESPACE}" 120 || true

    section "Reset Complete! 🏁"
    success "Cluster matches Helm chart - zero drift"
}

status() {
    banner
    section "Helm Test Track Status"
    ...
    helm list -n "${TEST_NAMESPACE}" --kube-context "$(get_context)" 2>/dev/null | sed 's/^/    /' || echo "    Not installed"
...
run_scan() {
    banner
    section "Running Dorikin Helm Scan"
    ...
    echo -e "Scanning Helm chart ${CYAN}${HELM_CHART_DIR}${NC}..."
    echo ""
...
run_tui() {
    banner
    section "Launching Dorikin Helm TUI"
...
${JADE_BOLD}COMMANDS${NC}
    ${CYAN}deploy${NC}     Deploy Helm chart to cluster
    ...
    ${CYAN}scan${NC}       Run dorikin scan with --helm
    ${CYAN}tui${NC}        Launch dorikin TUI with --helm
...
${JADE_BOLD}QUICK START${NC}
    $0 deploy             ${GRAY}# Install Helm chart${NC}
...
${JADE_BOLD}NAMESPACE${NC}
    ${CYAN}dorikin-pitlane${NC} - Isolated from main test track (dorikin-garage)
